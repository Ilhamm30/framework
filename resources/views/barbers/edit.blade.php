<x-app-layout>
    ...
    <x-slot name="subheader">
        Edit data barber yang dipilih
    </x-slot>

    <div class="py-6 max-w-4xl mx-auto sm:px-6 lg:px-8">
        <div class="card">
            <div class="card-header">
                <h3 class="text-lg font-semibold text-gray-900">Form Edit Barber</h3>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.barbers.update', $barber->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $barber->name) }}" class="form-input @error('name') border-red-500 @enderror" required>
                            @error('name')
                        ...
                        <div>
                            ...
                            <input type="text" name="specialty" value="{{ old('specialty', $barber->specialty) }}" class="form-input @error('specialty') border-red-500 @enderror" required>
                            @error('specialty')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="bio" rows="4" class="form-input @error('bio') border-red-500 @enderror">{{ old('bio', $barber->bio) }}</textarea>
                            @error('bio')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            @if($barber->image)
                                <div class="mt-2">
                                    ...
                                    <img src="{{ Storage::url($barber->image) }}" alt="{{ $barber->name }}" class="w-24 h-24 object-cover rounded-lg">
                                </div>
                            @endif
                        </div>
